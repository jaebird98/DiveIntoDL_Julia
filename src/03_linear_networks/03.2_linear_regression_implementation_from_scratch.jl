...
# ╔═╡ b0ed1998-701c-4495-a5dd-91fd232ee8f5
begin
	using Flux
	using Random
	using MLDatasets
	...
	# Gadfly skip. The fuck is it doing in this thing?
end

# ╔═╡ ee714cd6-5e2b-11ec-0b76-8b515eed06a8
md"""
# 3.2. Linear Regression Implementation from Scratch

We will be using flux for the entire implementation. There are some minute differences w.r.t. pytorch.
"""

# ╔═╡ e7813c80-caab-413e-a6f9-41afd04e20d5
# Generate dataset
begin
	function synth_data(w, b, n_ex)
		X = randn(n_ex, length(w))
		y = X * w .+ b
		y += 0.01 * randn(size(y))
		return X, y
	end
	...
	true_w = [2., -3.4]
	true_b = 4.2
	features, labels, = synth_data(true_w, true_b, 1000)
end

# ╔═╡ b99e5b3f-74f1-4fa4-8622-9379e5a8161b
md"""
...
Since this process is so fundamental to training machine learning algorithms, it is worth defining a utility function to shuffle the dataset and access it in minibatches.

In order to create a function that generates minibatches to iterate, python has a yield keyword for that.
While the most "Julian" way is somewhat different, since this subchapter is only a naive implementation and to stay close to the original material, we use the ResumableFunctions.jl module which is great but not production ready as of 2022-01-10.
...
# ╔═╡ 9b4ab645-902d-40f7-95b3-062d25867178
@resumable function data_iter(batch_size, features, labels)
	num_examples = size(features)[1]
	indices = Array(1:num_examples)

	shuffle!(indices)
	for i = 1:batch_size:num_examples
		batch_indices = indices[i: min(i + batch_size-1, num_examples)]
		@yield features[batch_indicies], labels[batch_indices]
	end
end

# ╔═╡ 2abbbc9a-8562-4a60-b191-f4ac367212ce
begin
	batch_size = 10
	data_iter(batch_size, features, labels)
end

# ╔═╡ 8594c227-49ab-47cf-bf95-66bb21afc213
md"""
## 3.2.3 Initializing Model Parameters

Before we can begin optimizing our model's parameters by minibatch stochastic gradient descent, we need to have some parameters in the first place. In the following code, 
